...
const TypesList = () => {
    ...
    const [typeRange, setTypeRange] = useState('home-office'); // Default range value
    ...
    const loadTypes = () => {
        // If "all" is selected, fetch all types, otherwise filter by selected range.
        fetchTypes()
            .then((data) => {
                if (selectedRange === 'all') {
                    setTypes(data);
                } else {
                    setTypes(data.filter(type => type.expenses_range === selectedRange));
                }
            })
